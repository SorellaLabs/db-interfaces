...
use super::utils::find_file_path;

pub(crate) fn postgres_table(token_stream: TokenStream) -> syn::Result<TokenStream> {
    let parsed: PostgresTableParse = syn::parse2(token_stream)?;
...
#[derive(Clone)]
pub(crate) struct PostgresTableParse {
...
impl PostgresTableParse {
    ...
    pub(crate) fn schema_name(&self) -> Ident {
        self.db_hierarchy.first().unwrap().clone()
    }

    pub(crate) fn schema_name_string(&self) -> String {
        self.schema_name().to_string().to_lowercase()
    }
...
impl PostgresTableParse {
    fn to_token_stream(self) -> syn::Result<TokenStream> {
        let this = self.clone();
        let PostgresTableParse { table_path, dbms, data_type, other_tables_needed, .. } = self;
        let other_tables_needed = other_tables_needed
            .into_iter()
            .map(|table| table.into_token_stream())
            .collect_vec();

        let TableMeta { table_name_str, db_table_type, schema_name, file_path } = TableMeta::new(this, table_path.as_ref())?;

        let (table_name_str, db_table_type, file_path, other_tables_needed) =
            (table_name_str, db_table_type, file_path.into_token_stream(), quote!(&[#(#dbms::#other_tables_needed),*]));

        let no_file_impls = if table_path.is_none() {
            quote! {
                fn create_table(_database: &::db_interfaces::posgres::client::PostgresClient<#dbms>)
        ...
        let val = quote! {
            impl ::db_interfaces::postgres::tables::PostgresTable<#dbms> for #db_table_type {
                ...
                type PostgresDataType = #data_type;
        ...
        let no_file_impls_test = if table_path.is_none() {
            quote! {
                fn create_test_table(_database: &::db_interfaces::postgres::test_utils::PostgresTestClient<#dbms>, _random_seed: u32)
                     -> impl std::future::Future<Output = Result<(), ::db_interfaces::errors::DatabaseError>> {
                    unreachable!("Not Enabled - No File Path Given In Macro");
                    async {}
                }
        ...
        #[cfg(feature = "test-utils")]
        let val_test = quote! {
            impl ::db_interfaces::postgres::test_utils::PostgresTestTable<#dbms> for #db_table_type {
                #no_file_impls_test
            }
        };
        #[cfg(feature = "test-utils")]
...
impl Parse for PostgresTableParse {
    fn parse(input: syn::parse::ParseStream<'_>) -> syn::Result<Self> {
        ...
        let data_type = if input.peek2(syn::Ident) {
            ...
            quote!(db_interfaces::postgres::types::NoneType)
        };
...
impl TableMeta {
    pub(crate) fn new(parsed: PostgresTableParse, table_path: Option<&LitStr>) -> syn::Result<Self> {
        let db_table_type = parsed.db_table_type();

        let table_name_str = parsed.table_name_string();
        let schema_name = parsed.schema_name_string();
        let file_path_str = find_file_path(&table_name_str, &schema_name, table_path);
        let file_path = LitStr::new(&file_path_str, Span::call_site());

        let this = Self { schema_name, table_name_str, db_table_type, file_path };
