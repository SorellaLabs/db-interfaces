...
impl RemoteClickhouseTableParse {
    fn to_token_stream(self) -> syn::Result<TokenStream> {
        ...
        let no_file_impls = if table_path.is_none() {
            quote! {
                    async fn create_table(_database: &::db_interfaces::clickhouse::client::ClickhouseClient<#dbms>) -> Result<(), ::db_interfaces::errors::DatabaseError> {
                        unreachable!("Not Enabled - No File Path Given In Macro")
                    }

                    async fn create_test_table(_database: &::db_interfaces::clickhouse::client::ClickhouseClient<#dbms>, _random_seed: u32) -> Result<(), ::db_interfaces::errors::DatabaseError> {
                        unreachable!("Not Enabled - No File Path Given In Macro")
                    }
            }
        } else {
