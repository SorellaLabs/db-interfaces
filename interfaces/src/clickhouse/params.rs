...
pub trait BindClickhouseParameters: Send + Sync {
    fn bind_query(&self, query: Query) -> Query;
}

impl<T: BindClickhouseParameters + Serialize> BindClickhouseParameters for &T {
    fn bind_query(&self, query: Query) -> Query {
        query.bind(self)
    }
}
...
macro_rules! impl_bind_parameters {
    ($($T:ty),*) => {
        $(
            impl BindClickhouseParameters for $T
            where
                Self: Bind + Serialize,
            {
                fn bind_query(&self, query: Query) -> Query {
...
impl BindClickhouseParameters for &str {
    fn bind_query(&self, query: Query) -> Query {
        query.bind(self)
    }
}
...
macro_rules! impl_generic_bind_parameters {
    ($($T:ty),*) => {
        $(
            impl<I> BindClickhouseParameters for $T
            where
                $T: Bind + Serialize,
...
macro_rules! impl_tuple_bind_parameters {
    ($($T:ident,)*) => {
        impl<$($T: Bind + Serialize + Debug + Send + Sync),*> BindClickhouseParameters for ($($T,)*) {
            #[allow(unused_mut)]
            fn bind_query(&self, query: Query) -> Query {
                let mut query = query;
