use super::{PostgresTestClient, PostgresTestDBMS};
use crate::{
    postgres::{
        errors::PostgresError,
        tables::{PostgresTable, PostgresTableKind}
    },
    errors::DatabaseError,
    Database
};

pub trait PostgresTestTable<D>: PostgresTable<D>
where
    D: PostgresTestDBMS + 'static
{
    /// FOR TESTING: creates the test table and associated test tables
    fn create_test_table(
        database: &PostgresTestClient<D>,
        random_seed: u32
    ) -> impl std::future::Future<Output = Result<(), DatabaseError>> + Send {
        async move {
            let table_sql_path = Self::FILE_PATH;
            let mut create_sql = std::fs::read_to_string(table_sql_path).map_err(|e| PostgresError::SqlFileReadError(e.to_string()))?;
            create_sql = Self::replace_test_str(create_sql);

            let table_type = Self::TABLE_TYPE;
            if matches!(table_type, PostgresTableKind::Distributed) {
    ...
    fn drop_test_db(database: &PostgresTestClient<D>) -> impl std::future::Future<Output = Result<(), DatabaseError>> + Send {
        async {
